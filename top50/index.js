...
function removeFeature() {
    alert("Select the feature you want to remove.")
    document.getElementById("removeFeature").classList.add("active");
    document.addEventListener("click", removeTheFeature);
}

function removeTheFeature(event) {
    if (event.target.id === "removeFeature" || !["INPUT","SELECT","TEXTAREA","BUTTON"].includes(event.target.tagName)) return;
    removeElement(event.target);
    document.removeEventListener("click", removeTheFeature);
    document.getElementById("removeFeature").classList.remove("active");
    if (["INPUT","SELECT","TEXTAREA"].includes(event.target.tagName)) {
        let ancestorIds = [];
        let currentElem = event.target.parentElement;
        while (currentElem) {
            ancestorIds.push(currentElem.id || currentElem.className);
            currentElem = currentElem.parentElement;
        }

        if (ancestorIds.includes("editSettings")) {
            if (event.target.nextElementSibling?.type === "checkbox") {
                removeElement(event.target.nextElementSibling);
                removeElement(event.target.nextElementSibling.nextElementSibling);
            }
        }

        if (event.target.id === "add_bg_color") {
        ...
        if (event.target.id === "edit_bg_color") {
            let elem = event.target.nextElementSibling;
            for (i = 0; i < 5; i++) {
                removeElement(elem);
                elem = elem.nextElementSibling;
        ...
        if (event.target.parentElement.tagName === "LABEL") {
            ...
            if (event.target.parentElement.nextElementSibling?.tagName === "BR") {
                removeElement(event.target.parentElement.nextElementSibling);
            }
        }

        if (event.target.previousElementSibling?.previousElementSibling?.tagName === "LABEL") {
            removeElement(event.target.previousElementSibling.previousElementSibling);
            removeElement(event.target.previousElementSibling);
        } else if (event.target.previousElementSibling?.tagName === "LABEL") {
            removeElement(event.target.previousElementSibling);
        }
        if (event.target.nextElementSibling?.tagName === "LABEL" || event.target.nextElementSibling?.tagName === "BR") {
            removeElement(event.target.nextElementSibling);
        }

        if (ancestorIds.includes("boringSettings")) {
            setTimeout(() => {
                if (!event.target.parentElement.innerText.replaceAll("\n", "")) {
    ...
    if (event.target.id === "pauseB") {
        if (data.pause) pause();
    }

    if (event.target.classList.contains("settingType")) {
        removeElement(document.getElementById(event.target.id.replace("button_", "")));
    }
