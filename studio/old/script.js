...
let user = localStorage.getItem("user-studio-v6-9") ? JSON.parse(localStorage.getItem("user-studio-v6-9")) : {
	name: "Loading",
	image: "../../default.png",
	footer: "Subscribers",
	count: 0,
...
function saveData() {
	localStorage.setItem("user-studio-v6-9", JSON.stringify(user))
	alert("Saved!")
}
...
function selectThing(id) {
	for (let q = 0; q < importedChannels.data.length; q++) {
		if (id == importedChannels.data[q].id) {
			...
			localStorage.setItem("user-studio-v6-9", JSON.stringify(channel))
			location.reload()
		}
	}
}

if (localStorage.getItem("user-studio-v6-9")) {
	console.log(user)
...
function resetData() {
	if (confirm("Are you sure you want to reset all data? This cannot be undone.")) {
		localStorage.removeItem("user-studio-v6-9")
		location.reload()
	}
}